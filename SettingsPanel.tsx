...
export function SettingsPanel({
  ...
  return (
    <aside className="anim-slidein pointer-events-auto absolute inset-y-0 right-0 z-40 w-[min(330px,90vw)] border-l border-line bg-deck/95 backdrop-blur-md">
      <div className="flex h-12 items-center justify-between border-b border-line px-4">
        <div>
          <div className="font-display text-[11px] uppercase tracking-[0.2em] text-[#d8dee3]">
            Console settings
          </div>
          <div className="mt-0.5 text-[8px] uppercase tracking-[0.18em] text-ash/55">Station 07 / local</div>
        </div>
        <button onClick={onClose} className="tool-button -mr-4" aria-label="Close settings">
          <X size={15} />
        </button>
      </div>
      ...
      <div className="px-4 py-5">
        ...
        <div className="border-b border-line py-4">
          ...
          <input
            ...
            className="h-1 w-full cursor-pointer appearance-none bg-line accent-cool"
          />
          <div className="mt-2 flex justify-between text-[8px] uppercase tracking-[0.15em] text-ash/45">
            <span>Still</span>
        ...
        <fieldset className="border-b border-line py-4">
          <legend className="mb-2 text-[10px] uppercase tracking-[0.16em] text-ash">Thermal palette</legend>
        ...
        <p className="mt-4 text-[9px] leading-4 text-ash/55">
          Configuration is stored on this device. Reduced-motion system preferences are respected automatically.
        </p>
      </div>
    </aside>
  );
